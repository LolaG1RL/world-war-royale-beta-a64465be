    ...
    return (
      <div className="h-screen w-full max-w-md mx-auto flex flex-col bg-background overflow-hidden">
        <div className="flex items-center gap-2 px-3 py-2 bg-[hsl(220,25%,12%)] border-b border-border">
          ...
          <h2 className="font-display font-bold text-foreground text-sm">{t('river.title', language)}</h2>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          <Ship className="w-16 h-16 text-muted-foreground/30 mb-4" />
          <div className="text-sm font-display font-bold text-foreground mb-1">{t('river.join_clan_first', language)}</div>
          <div className="text-xs text-muted-foreground text-center mb-4">{t('river.need_clan', language)}</div>
          <button onClick={() => setScreen('social')} className="btn-battle text-xs px-6 py-2">{t('river.go_to_social', language)}</button>
        </div>
