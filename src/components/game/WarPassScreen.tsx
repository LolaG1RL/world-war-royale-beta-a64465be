...
const WAR_PASS_REWARDS: PassReward[] = Array.from({ length: 60 }, (_, i) => {
  const tier = i + 1;
  const crownsNeeded = tier * 10;

  // Free track rewards rotation
  const freeOptions = [
    { type: 'gold', amount: 100 + tier * 50, emoji: '💰', label: `${100 + tier * 50} Gold` },
    { type: 'cards', amount: Math.max(2, Math.floor(tier / 5)), emoji: '🃏', label: `${Math.max(2, Math.floor(tier / 5))} Cards` },
    { type: 'gems', amount: 3 + Math.floor(tier / 3), emoji: '💎', label: `${3 + Math.floor(tier / 3)} Gems` },
    { type: 'chest', amount: 1, emoji: tier > 40 ? '⚡' : tier > 20 ? '✨' : tier > 10 ? '💰' : '🪙', label: tier > 40 ? 'Lightning Chest' : tier > 20 ? 'Magic Chest' : tier > 10 ? 'Gold Chest' : 'Silver Chest' },
  ];
  // Paid track rewards rotation
  const paidOptions = [
    { type: 'gems', amount: 5 + tier * 3, emoji: '💎', label: `${5 + tier * 3} Gems` },
    { type: 'gold', amount: 300 + tier * 100, emoji: '💰', label: `${((300 + tier * 100) / 1000).toFixed(1)}K Gold` },
    { type: 'chest', amount: 1, emoji: tier > 40 ? '👑' : tier > 20 ? '⚡' : '✨', label: tier > 40 ? 'Legendary Chest' : tier > 20 ? 'Lightning Chest' : 'Magic Chest' },
    { type: 'cards', amount: Math.max(2, Math.floor(tier / 8)), emoji: '🃏', label: `${Math.max(2, Math.floor(tier / 8))} Epic Cards` },
  ];

  let free = freeOptions[i % freeOptions.length];
  let paid = paidOptions[i % paidOptions.length];

  // Special milestone overrides for legendary chests on free track
  if (tier % 10 === 0 && tier <= 50) {
    free = { type: 'chest', amount: 1, emoji: '👑', label: 'Legendary Chest' };
  }

  // ★ Tier 20 & 30 Pass+ = Exclusive Emotes
  if (tier === 20) paid = { type: 'emote', amount: 1, emoji: '🎭', label: 'Exclusive Emote' };
  if (tier === 30) paid = { type: 'emote', amount: 1, emoji: '👑', label: 'Champion Emote' };
  // ★ Tier 50 Pass+ = Season Exclusive Emote
  if (tier === 50) paid = { type: 'season-emote', amount: 1, emoji: '🎭', label: 'S1 Emote' };
  // ★ Tier 55 Pass+ = Season Exclusive Banner Set
  if (tier === 55) paid = { type: 'season-banner', amount: 2, emoji: '🎨', label: 'S1 Banner Set' };
  // ★ Tier 60 Pass+ = Season Exclusive Champion
  if (tier === 60) paid = { type: 'season-hero', amount: 1, emoji: '👑', label: 'Joan of Arc' };
